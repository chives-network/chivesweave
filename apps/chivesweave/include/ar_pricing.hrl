...
%% The fixed USD to XWE rate used after the fork 2.6 until the automatic transition to the new
...
-else.
	-ifndef(PRICE_2_6_8_TRANSITION_START).
		...
		-else.
			-define(PRICE_2_6_8_TRANSITION_START, (0)). % ~6.5 months / Dec. 14, 2023
		-endif.
	-endif.
-endif.
...
-else.
	-ifndef(PRICE_2_6_8_TRANSITION_BLOCKS).
		...
		-else.
			-ifndef(PRICE_2_6_8_TRANSITION_BLOCKS).
				-define(PRICE_2_6_8_TRANSITION_BLOCKS, (0)). % ~18 months.
			-endif.
		-endif.
	-endif.
-endif.
...
%% The assumed annual decay rate of the Chivesweave prices, expressed as a decimal fraction.
...
%% Re-denominate XWE (multiply by 1000) when the available supply falls below this
%% number of units.
-ifdef(DEBUG).
...
%% USD to XWE exchange rates by height defined together with INITIAL_USD_TO_AR_HEIGHT
%% and INITIAL_USD_TO_AR_DIFF. The protocol uses these constants to estimate the
%% USD to XWE rate at any block based on the change in the network difficulty and inflation
%% rewards.
...
%% The original USD to XWE conversion rate, defined as a fraction. Set up at fork 2.4.
%% Used until the fork 2.5.
-define(INITIAL_USD_TO_AR_PRE_FORK_2_5, {1, 5}).

%% The network difficulty at the time when the USD to XWE exchange rate was
%% ?INITIAL_USD_TO_AR(Height). Used to account for the change in the network
%% difficulty when estimating the new USD to XWE rate.
...
%% The network height at the time when the USD to XWE exchange rate was
%% ?INITIAL_USD_TO_AR(Height). Used to account for the change in inflation
%% rewards when estimating the new USD to XWE rate.
...
%% The amount in XWE depends on the current difficulty and height.
...
%% The largest possible multiplier for a one-step increase of the USD to XWE Rate.
-define(USD_TO_AR_MAX_ADJUSTMENT_UP_MULTIPLIER, {1005, 1000}).

%% The largest possible multiplier for a one-step decrease of the USD to XWE Rate.
-define(USD_TO_AR_MAX_ADJUSTMENT_DOWN_MULTIPLIER, {995, 1000}).

%% Reduce the USD to XWE fraction if both the dividend and the devisor become bigger than this.
-ifdef(DEBUG).
...
%% The USD to XWE exchange rate for a new chain, e.g. a testnet.
-define(NEW_WEAVE_USD_TO_AR_RATE, ?INITIAL_USD_TO_AR_PRE_FORK_2_5).

%% Initial $/XWE exchange rate. Used until the fork 2.4.
