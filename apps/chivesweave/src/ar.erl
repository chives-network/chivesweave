%%%
%%% @doc Chivesweave server entrypoint and basic utilities.
%%%
-module(ar).
...
show_help() ->
	io:format("Usage: chivesweave-server [options]~n"),
	...
	lists:foreach(
		...
		[
			...
			{"randomx_bulk_hashing_iterations",
				...
				" to the Chivesweave miner. The faster CPU hashes, the higher this value should be."
			},
			{"disk_cache_size_mb",
				lists:flatten(io_lib:format(
...
%% @doc Start an Chivesweave node on this BEAM.
start() ->
...
start(normal, _Args) ->
	...
	logger:set_application_level(chivesweave, Level),
...
set_mining_address(#config{ mining_addr = not_set } = C) ->
	...
	application:set_env(chivesweave, config, C2),
...
create_wallet([DataDir]) ->
	case filelib:is_dir(DataDir) of
		false ->
			create_wallet_fail();
		true ->
			ok = application:set_env(chivesweave, config, #config{ data_dir = DataDir }),
...
stop_dependencies() ->
	{ok, [_Kernel, _Stdlib, _SASL, _OSMon | Deps]} = application:get_key(chivesweave, applications),
...
docs() ->
	Mods =
		lists:filter(
			...
			element(2, file:list_dir("apps/chivesweave/src"))
		),
	edoc:files(
		["apps/chivesweave/src/" ++ Mod || Mod <- Mods],
		[
