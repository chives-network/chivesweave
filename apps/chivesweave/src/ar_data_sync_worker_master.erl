...
test_cut_peer_queue() ->
	{ok, OriginalConfig} = application:get_env(chivesweave, config),
	try
		ok = application:set_env(chivesweave, config, OriginalConfig#config{
			sync_jobs = 10
		}),
	...
	after
		application:set_env(chivesweave, config, OriginalConfig)
	end.
