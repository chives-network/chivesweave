...
-include_lib("chivesweave/include/ar_header_sync.hrl").
-include_lib("chivesweave/include/ar_data_sync.hrl").
-include_lib("chivesweave/include/ar_chunk_storage.hrl").
...
handle_cast({remove_tx, TXID}, State) ->
	...
	xwe_tx_blacklist:notify_about_removed_tx(TXID),
	{noreply, State};
...
add_block(B, State) ->
	case check_fork(B#block.height, B#block.indep_hash, B#block.tx_root) of
		false ->
			{ok, State};
		true ->
			case B#block.height == 0 andalso ?NETWORK_NAME == "chivesweave.mainnet.1" of
				true ->
					ok;
				false ->
