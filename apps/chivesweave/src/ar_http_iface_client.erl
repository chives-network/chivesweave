%%%
%%% @doc Exposes access to an internal Chivesweave client to external nodes on the network.
%%%
...
send_tx_json(Peer, TXID, Bin) ->
	ar_http:req(#{
		...
		peer => Peer,
		path => "/tx",
		headers => [{<<"chivesweave-tx-id">>, ar_util:encode(TXID)} | p2p_headers()],
		body => Bin,
...
send_block_json(Peer, H, Payload) ->
	ar_http:req(#{
		...
		peer => Peer,
		path => "/block",
		headers => [{<<"chivesweave-block-hash">>, ar_util:encode(H)} | p2p_headers()],
		body => Payload,
		connect_timeout => 5000,
		timeout => 20 * 1000
	}).
...
send_block_binary(Peer, H, Payload, RecallByte) ->
	...
	Headers2 = case RecallByte of undefined -> Headers; _ ->
			[{<<"chivesweave-recall-byte">>, integer_to_binary(RecallByte)} | Headers] end,
	ar_http:req(#{
		method => post,
		peer => Peer,
		path => "/block2",
