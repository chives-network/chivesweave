...
test_sign_and_verify_chunked() ->
	...
	Args = {Rate, PricePerGiBMinute, 1, 1, 0, Height,
			maps:from_list([{Address, {?XWE(100), <<>>}}]), Timestamp},
...
test_check_last_tx() ->
	...
	TX = ar_tx:new(Pub2, ?XWE(1), ?XWE(500), <<>>),
	TX2 = ar_tx:new(Pub3, ?XWE(1), ?XWE(400), TX#tx.id),
	TX3 = ar_tx:new(Pub1, ?XWE(1), ?XWE(300), TX#tx.id),
