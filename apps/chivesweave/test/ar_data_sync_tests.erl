...
test_fork_recovery(Split) ->
	...
	{TX1, Chunks1} = tx(Wallet, {Split, 13}, v2, ?XWE(10)),
	...
	{SlaveTX2, SlaveChunks2} = tx(Wallet, {Split, 15}, v2, ?XWE(10)),
	{SlaveTX3, SlaveChunks3} = tx(Wallet, {Split, 17}, v2, ?XWE(10)),
	...
	{MasterTX2, MasterChunks2} = tx(Wallet, {Split, 14}, v2, ?XWE(10)),
	...
	{SlaveTX4, SlaveChunks4} = tx(Wallet, {Split, 22}, v2, ?XWE(10)),
	...
	{MasterTX3, MasterChunks3} = tx(Wallet, {Split, 16}, v2, ?XWE(10)),
...
test_syncs_after_joining(Split) ->
	{Master, Slave, Wallet} = setup_nodes(),
	{TX1, Chunks1} = tx(Wallet, {Split, 17}, v2, ?XWE(1)),
	B1 = post_and_mine(#{ miner => {master, Master}, await_on => {slave, Slave} }, [TX1]),
	Proofs1 = post_proofs_to_master(B1, TX1, Chunks1),
	UpperBound = ar_node:get_partition_upper_bound(ar_node:get_block_index()),
	slave_wait_until_syncs_chunks(Proofs1, UpperBound),
	wait_until_syncs_chunks(Proofs1),
	disconnect_from_slave(),
	{MasterTX2, MasterChunks2} = tx(Wallet, {Split, 13}, v2, ?XWE(1)),
	MasterB2 = post_and_mine(
		#{ miner => {master, Master}, await_on => {master, Master} },
		[MasterTX2]
	),
	MasterProofs2 = post_proofs_to_master(MasterB2, MasterTX2, MasterChunks2),
	{MasterTX3, MasterChunks3} = tx(Wallet, {Split, 12}, v2, ?XWE(1)),
	MasterB3 = post_and_mine(
		#{ miner => {master, Master}, await_on => {master, Master} },
		[MasterTX3]
	),
	MasterProofs3 = post_proofs_to_master(MasterB3, MasterTX3, MasterChunks3),
	{SlaveTX2, SlaveChunks2} = tx(Wallet, {Split, 20}, v2, ?XWE(1)),
...
setup_nodes(MasterAddr, SlaveAddr) ->
	...
	[B0] = ar_weave:init([{ar_wallet:to_address(Pub), ?XWE(200000), <<>>}]),
	{ok, Config} = application:get_env(chivesweave, config),
...
%% @doc Split the way v2 transactions are usually split (chivesweave-js does it
