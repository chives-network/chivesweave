...
test_reject_block_invalid_double_signing_proof() ->
	...
	[B0] = ar_weave:init([{Addr0, ?XWE(1000), <<>>}], ar_retarget:switch_to_linear_diff(2)),
	start(B0),
	slave_start(B0),
	disconnect_from_slave(),
	{ok, Config} = slave_call(application, get_env, [chivesweave, config]),
	...
	TX0 = sign_tx(Key0, #{ target => ar_wallet:to_address(Key), quantity => ?XWE(10) }),
	...
	{ok, MasterConfig} = application:get_env(chivesweave, config),
...
test_send_block2() ->
	...
	[B0] = ar_weave:init([{ar_wallet:to_address(Pub), ?XWE(100), <<>>}]),
