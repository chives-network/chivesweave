...
test_uses_blacklists() ->
	...
	{_, _} =
		start(B0, RewardAddr,
				(element(2, application:get_env(chivesweave, config)))#config{
...
setup() ->
	...
	V1TX = sign_v1_tx(Wallet, #{ data => random_v1_data(3 * ?DATA_CHUNK_SIZE),
			last_tx => get_tx_anchor(slave), reward => ?XWE(10000) }),
...
setup_slave() ->
	...
	[B0] = ar_weave:init([{ar_wallet:to_address(Pub), ?XWE(100000000), <<>>}]),
	slave_start(B0),
	{B0, Wallet}.
		...
		fun
			(_, {TXs, DataTrees}) ->
				...
				TX = sign_tx(Wallet, #{ format => 2, data_root => DataRoot,
						...
						reward => ?XWE(10000), denomination => 1 }),
...
random_filename() ->
	{ok, Config} = slave_call(application, get_env, [chivesweave, config]),
...
teardown() ->
	...
	application:set_env(chivesweave, config, Config#config{
